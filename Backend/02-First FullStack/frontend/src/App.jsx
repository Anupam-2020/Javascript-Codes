...
function App() {
  ...
  useEffect(() => {
    ...
    // we need to apply proxy(vite.config.js file) to allow the access of backend in our frontend.
    ...
    axios.get('/api/jokes') // We donot need to write whole url to fetch data, only endpoints are sufficient. We put the base url in proxy(vite.config.js)...
    .then((response) => {
